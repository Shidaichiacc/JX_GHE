...
local szTitle =myTitleText
	...
	{		
		{"BiÕt råi, ®Ó ta hoµn thµnh nhiÖm vô xong míi l¹i t×m ng­¬i",Task_Wait},
		{"Ta ®· hoµn thµnh nhiÖm vô lÇn nµy, xin h·y kiÓm tra l¹i!",Task_Accept},
		{"[ Hoµn Thµnh Nhanh 5 v¹n :d ]",Task_FinishCheat},
...
function Task_FinishCheat()
	...
	if (GetCash() >= nMoney) then
		...
		Talk(1, "", "Ng­¬i kh«ng cã " ..floor(nMoney/10000).. " v¹n kh«ng thÓ hoµn thµnh nhiÖm vô D· TÈu")
	end
end
...
function Task_GiveAward()
	...
	if (nTotalTaskNum ~= 0 and mod(nTotalTaskNum, 10) == 0 and GetTask(TKS_TASKLINK_SPITEM) ~= nTotalTaskNum) then
		...
		local tbItem = {tbProp = {6, 1, 4905, 1, 0, 0}}
		local tbItem1 = {
			tbProp = {6, 1, 4903, 1, 0, 0},
			nCount = datauvll,         -- s? l??ng
			nBindState = -2       -- khóa v?t ph?m
		}
		tbAwardTemplet:GiveAwardByList(tbItem, "seasonnpc_10task")--bao ruong thang bi
		tbAwardTemplet:GiveAwardByList(tbItem1, "seasonnpc_10task")--vll
