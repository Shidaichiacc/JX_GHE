...
function HocKyNangMonPhai()
	...
	if (getn(TAB_SKILLALL[nFaction].tbSkill9x) > 0) then
		for i = 1, getn(TAB_SKILLALL[nFaction].tbSkill9x) do
			if (skillsupport(TAB_SKILLALL[nFaction].tbSkill9x[i]) == 1) then
				...
				if (HaveMagic(TAB_SKILLALL[nFaction].tbSkill9x[i]) == -1) then--skill 9x,lv
					AddMagic(TAB_SKILLALL[nFaction].tbSkill9x[i], 1)
				end
			end
		end
	end
	if (getn(TAB_SKILLALL[nFaction].tbSkill12x) > 0) then --skill 12x,lv
	...
	if (getn(TAB_SKILLALL[nFaction].tbSkill15x) > 0) then --skill 15x-lv
		for i = 1, getn(TAB_SKILLALL[nFaction].tbSkill15x) do
			if (HaveMagic(TAB_SKILLALL[nFaction].tbSkill15x[i]) == -1) then
				AddMagic(TAB_SKILLALL[nFaction].tbSkill15x[i],25)
			end
		end
	end
end
