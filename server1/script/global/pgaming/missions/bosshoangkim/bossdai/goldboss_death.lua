...
function OnDeath(nBossIndex)
	...
	local totaldrop = random(15,20);-- slo luong do xanh
	...
	AddDropItemBlue(nBossIndex,totaldrop,nNpcSeries,random(8,10),GetLuckyPlayer());-- cap do  do xanh
	DropQuestItemBoss(nBossIndex,OrgPlayer) -- rot do pho   do hkmp
	--DropBigBoss_DoPho(nBossIndex)---rot manh hk
...
function DropBigBoss_DoPho(nNpcIndex) --- rot manh hk
	local tbDoPho = 
	{
...
function DropQuestItemBoss(nNpcIndex,OrgPlayer)
	...
	if type(item) == "table" then 
		...
		-- rot  vlmt  ...  szName khai bao ben goldboss_h.lua
		for k=1,rloop do 
